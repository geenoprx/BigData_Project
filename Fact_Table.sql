...
CREATE OR REPLACE PACKAGE BODY PKG_LOAD_FACT_REPORTS AS
    ...
    -- -------------------------------------------------------
    -- Appends new bookings to FactRevenueAnalysis.
    -- Only loads rows with DateKey greater than the current max (high watermark).
    -- GrossProfit = TotalRevenue - AllocatedCost, where cost is distributed
    -- proportionally across bookings by pax share (BookingPax / TourTotalPax).
    -- -------------------------------------------------------
    ...
    -- -------------------------------------------------------
    -- Appends new tours to FactTripPerformance.
    -- Revenue excludes CANCELLED bookings. Cost sums all CostDetail rows.
    -- Guide is resolved from the first non-null GuideID in CostDetail.
    -- -------------------------------------------------------
    ...
    -- -------------------------------------------------------
    -- Appends new bookings to FactBookingStatus.
    -- CancelCount is 1 if the booking was cancelled, otherwise 0.
    -- -------------------------------------------------------
