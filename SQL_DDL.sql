...
CREATE TABLE
    Department (
        DeptCode VARCHAR2 (8) PRIMARY KEY,
...
CREATE TABLE
    Employee (
        ...
        CONSTRAINT CK_Employee_Status CHECK (Status IN ('A', 'R')), --A = Active, R = Resigned
        CONSTRAINT CK_Employee_ResignDate CHECK (
            ResignDate IS NULL
            OR (
                StartDate IS NOT NULL
                AND ResignDate >= StartDate
            )
        )
    );
...
FOR EACH ROW
DECLARE v_next_dayno NUMBER;
BEGIN
	IF :NEW.DayNo IS NULL THEN
		SELECT NVL(MAX(DayNo), 0) + 1 INTO v_next_dayno FROM TourDetail WHERE TourID = :NEW.TourID;
...
CREATE TABLE
    CustomerType (
        ...
        updatedAt TIMESTAMP,
        updatedBy VARCHAR2 (128),
        CONSTRAINT CK_CustomerType_DiscountRate CHECK (
            DiscountRate >= 0
            AND DiscountRate <= 100
        )
    );
...
FOR EACH ROW
DECLARE v_next_seqno NUMBER;
BEGIN
	IF :NEW.SeqNo IS NULL THEN
		SELECT NVL(MAX(SeqNo), 0) + 1 INTO v_next_seqno FROM BookingDetail WHERE BookingID = :NEW.BookingID;
...
BEGIN
	IF :NEW.SeqNo IS NULL THEN
		SELECT NVL(MAX(SeqNo), 0) + 1 INTO v_next_seqno FROM CostDetail WHERE TourID = :NEW.TourID;
		:NEW.SeqNo := v_next_seqno;
	END IF;
END;
...
CREATE TABLE
    PromotionDetail (
        ...
        StartDate DATE NOT NULL,
        EndDate DATE NOT NULL,
        ExtraCondition VARCHAR(255) NULL,
...
BEGIN
	IF :NEW.SeqNo IS NULL THEN
		SELECT NVL(MAX(SeqNo), 0) + 1 INTO v_next_seqno FROM PromotionDetail WHERE PromotionID = :NEW.PromotionID;
		:NEW.SeqNo := v_next_seqno;
	END IF;
END;